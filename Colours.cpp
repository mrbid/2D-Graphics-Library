...
clr::mColour clr::HSLtoRGB(mColour c)
{
	...
	const float Rt3 = math::wrap(c.r + 0.3333333333333333f);
	const float Gt3 = math::wrap(c.r);
	const float Bt3 = math::wrap(c.r - 0.3333333333333333f);

	return clr::mColour((6.0f * Rt3 < 1) ? t1 + (t2-t1) * 6.0f * Rt3 : (2.0f * Rt3 < 1) ? t2 : (3.0f * Rt3 < 2) ? t1 + (t2-t1) * (0.6666666666666667f - Rt3) * 6.0f : t1,
						 (6.0f * Gt3 < 1) ? t1 + (t2-t1) * 6.0f * Gt3 : (2.0f * Gt3 < 1) ? t2 : (3.0f * Gt3 < 2) ? t1 + (t2-t1) * (0.6666666666666667f - Gt3) * 6.0f : t1,
						 (6.0f * Bt3 < 1) ? t1 + (t2-t1) * 6.0f * Bt3 : (2.0f * Bt3 < 1) ? t2 : (3.0f * Bt3 < 2) ? t1 + (t2-t1) * (0.6666666666666667f - Bt3) * 6.0f : t1);
}
...
clr::mColour clr::RGBtoYUV(mColour c)
{
	...
	const float y = 0.299f * c.r + 0.587f * c.g + 0.114f * c.b;
	return clr::mColour(y,
						0.436f * ((c.b - y) * 1.1286681715575620767494356659142f),
						0.615f * ((c.r - y) * 1.4265335235378031383737517831669f));
}
...
clr::mColour clr::YUVtoRGB(mColour c)
{
	...
	return clr::mColour(c.r + c.b * 1.1398373983739837398373983739833f,
						c.r - c.g * (0.39465170435897035149961708578841f - c.b * 0.58059860666749768008753341366463f),
						c.r + c.g * 2.10321100917431192660550458715596f);
}
